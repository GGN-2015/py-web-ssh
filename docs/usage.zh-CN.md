...
终端侧的 Directory 面板使用同一份 CWD Sync 目录列表。普通文件会显示 `Download` 和 `Delete`；点击 `Delete` 会先弹出应用内确认框，确认后向 shell 发送可见的 `rm -- '文件名'` 命令。目录项会显示 `Enter Dir`，点击后发送可见的 `cd '目录名'` 命令。只有当后端确认远端会话已经回到 shell 提示符时，`UP`、`Enter Dir` 和 `Delete` 才可用；用户正在运行其他终端程序或正在输入命令时，这些按钮会保持禁用。
