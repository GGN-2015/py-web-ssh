...
使用 `--block-size` 可以设置上传命令大小的初始探测值。未写单位时按字节处理，也支持 `B`、`KB`、`MB`、`GB` 和 `TB` 后缀。
...
上传命令大小探测默认从 1 MiB 开始，也可以通过 `--block-size` 修改。Files 面板允许用户用 `TB`、`GB`、`MB`、`KB` 或 `B` 单位输入任意正整数。前端会把小于 `64 B` 的值强制改为 `64 B`；后端也会执行 `64 B` 的最小值限制。如果初始探测因为命令过长或连接关闭失败，py-web-ssh 会二分向下探测，并把最终选择的大小写入日志。

上传进度条会拆成两个加权阶段。从浏览器上传到服务端的进度占前 `30%`；服务端再通过 SSH 上传到远端服务器的进度占后 `70%`。
...
终端侧的 Directory 面板使用同一份 CWD Sync 目录列表。普通文件会显示 `Download` 和 `Delete`；目录会显示 `Enter Dir` 和 `Delete`。删除文件会先弹出应用内确认框，确认后向 shell 发送可见的 `rm -- '文件名'` 命令。删除目录会先询问确认，然后发送可见的 `rm -rf -- '目录名'` 命令。目录项的 `Enter Dir` 会发送可见的 `cd '目录名'` 命令。`Refresh` 会向 shell 发送可见的 `ls -al` 命令，并使用这段显示出来的输出重建 Directory 表格。只有当后端确认远端会话已经回到 shell 提示符时，`UP`、`Refresh`、`Enter Dir` 和 `Delete` 才可用；用户正在运行其他终端程序或正在输入命令时，这些按钮会保持禁用。

Directory 面板会把条目拆成上下两个等高表格：可见文件与隐藏文件。任何名称以 `.` 开头的条目都会被视为隐藏文件。文件上传成功后，以及文件删除命令执行完并回到就绪 shell 提示符后，py-web-ssh 会通过隐藏的 CWD Sync 通道静默刷新当前目录列表。

当 SSH session state 变为 `closed` 后，Directory 面板会清空当前目录、目录列表、加载状态和错误信息，恢复到初始空状态。
