...
The terminal-side Directory panel uses the same CWD Sync listing. Regular files show `Download` and `Delete`; directories show `Enter Dir` and `Delete`. File deletion opens an in-app confirmation dialog and, after confirmation, sends a visible `rm -- 'file-name'` command to the shell. Directory deletion asks for confirmation and then sends a visible `rm -rf -- 'directory-name'` command. Directory entry uses `Enter Dir` to send a visible `cd 'directory-name'` command. `Refresh` sends a visible `ls -al` command to the shell and uses that displayed output to rebuild the Directory tables. `UP`, `Refresh`, `Enter Dir`, and `Delete` are enabled only when the backend has confirmed that the remote session is back at a shell prompt, so they are disabled while another terminal program is active or while the user is typing a command.
